...
def gen_A(num_classes, t, adj_file):
    ...
    # 两类共现次数对角矩阵(classes_num, classes_num)
    _adj = result['adj']
    # 每个类的数量(classes_num)
    _nums = result['nums']
...
class GCNVAN(nn.Module):
    def __init__(self, model, num_classes, t=0.3, adj_file=None, in_channel=300, vec_file=None):
        super(GCNVAN, self).__init__()
        self.features = model
        self.num_classes = num_classes
        self.pooling = nn.MaxPool2d(7, 7)

        self.gc1 = GraphConvolution(in_channel, 1024)
        self.gc2 = GraphConvolution(1024, self.features.embed_dims[-1])
        self.relu = nn.LeakyReLU(0.2)
        # 初始化标签共现矩阵
        _adj = gen_A(num_classes, t, adj_file)
        self.A = Parameter(torch.from_numpy(_adj).float())
        # 加载词向量
        self.vec = torch.Tensor(pickle.load(open(vec_file, 'rb'))).to(config.device)
...
def gcn_van_b0(pretrained=False, in_chans=3, num_classes=1000):
    model = van_b0(pretrained=pretrained, in_chans=in_chans, num_classes=num_classes)
    t = 0.3
    in_channel = 300
    adj_file = f'./pkls/{config.datasets}_adj.pkl'
    vec_file = f'./pkls/{config.datasets}_glove_wordEmbedding.pkl'
    return GCNVAN(model, num_classes, t, adj_file, in_channel, vec_file)
