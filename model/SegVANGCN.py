...
class SegVANGCN(nn.Module):
    def __init__(self, pretrained=False, in_chans=3, num_classes=5):
        ...
        self.gcn = GCNNet(num_classes=num_classes, out_channel=self.net.embed_dims[-1], t=0.3)

    def forward(self, x):
        # seg
        with torch.no_grad():
            pred = self.unet(x).detach()
            seg_img = torch.argmax(pred, dim=1).unsqueeze(1)
            seg_img[seg_img > 0] = 1
            seg_img = seg_img * x
...
if __name__ == '__main__':
    model = SegVANGCN(num_classes=len(config.class_labels))
